...
export class ContractFunctionParameterBuilder {
  ...
  buildHAPIParams() {
    ...
    for (const param of this.params) {
      ...
      // Convert hex bytes32 string to Uint8Array — addBytes32 expects Uint8Array
      let value = param.value;
      if (param.type === 'bytes32' && typeof value === 'string' && value.startsWith('0x')) {
        value = new Uint8Array(Buffer.from(value.slice(2).padStart(64, '0'), 'hex'));
      }

      if (functionName in contractFunctionParams) {
        contractFunctionParams[functionName](value);
      } else {
